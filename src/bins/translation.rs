...
use torch::translation::dataset::Dataset;
use torch::translation::lang::Lang;
...
pub fn main() -> Result<()> {
    ...
    match args[1].as_str() {
        "train" => {
            ...
            for idx in 1..=SAMPLES {
                ...
                if idx % 10000 == 0 {
                    ...
                    if let Err(err) = vs.save(format!("translation{idx}.ot")) {
        ...
        "predict" => {
            ...
            let input_index = torch::translation::dataset::to_indexes(&torch::translation::dataset::normalize(input), &ilang);
            println!("Input sentence: {}", input);
            let predict = model.predict(&input_index);
            println!("tranlated sentence: {}", olang.seq_to_string(&predict));
        }
        _ => bail!("usage: main (train|predict weights.ot words_input)"),
    }
